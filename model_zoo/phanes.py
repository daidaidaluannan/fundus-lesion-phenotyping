...
class AnomalyMap:
    ...
    def compute_residual(self, x_rec, x):
        ...
        for batch_id in range(x_rec.size(0)):
            x_np = x[batch_id].cpu().detach().numpy()
            x_rec_np = x_rec[batch_id].cpu().detach().numpy()
            try:
                x_rescale = torch.Tensor(exposure.equalize_adapthist(x_np)).to(x_rec.device)
            except Exception:
                x_rescale = torch.Tensor(x_np).to(x_rec.device)
            try:
                x_rec_rescale = torch.Tensor(exposure.equalize_adapthist(x_rec_np)).to(x.device)
            except Exception:
                x_rec_rescale = torch.Tensor(x_rec_np).to(x.device)
